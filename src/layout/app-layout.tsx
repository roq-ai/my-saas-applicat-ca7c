...
import { FiMail, FiUsers, FiMenu, FiUser, FiMessageCircle, FiFile, FiBox, FiGitBranch } from 'react-icons/fi';
...
import { getBlockchains } from 'apiSdk/blockchains';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const { data, error, isLoading } = useSWR<BlockchainInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/blockchains` : null),
    () => getBlockchains({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Blockchain',
      path: isTenantUser ? `/blockchains/view/${data?.[0]?.id}` : '/blockchains',
      entity: 'blockchain',
